...
def get_story_components(config_path,story_title, story_summary, author, year, protagonist, 
    ...
    if story_components['protagonist'] != protagonist:
        print("⚠️ WARNING: LLM designated protagonist as: ", story_components['protagonist'], " but I specified protagonist as: ", protagonist)
        print("PLEASE INVESTIGATE")
        #raise ValueError

    #check if right author was chosen
    if story_components['title'] != story_title:
        print("⚠️ WARNING: LLM designated title as: ", story_components['title'], " but I specified title as: ", story_title)
        print("PLEASE INVESTIGATE")
        #raise ValueError
...
"""
  ...
  config = load_config(config_path=config_path)
  llm = get_llm(llm_provider, llm_model, config, max_tokens=16384) # Increased tokens for the more detailed analysis
  runnable = prompt | llm
  output = runnable.invoke({
