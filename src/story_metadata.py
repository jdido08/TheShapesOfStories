...
def _loc_suggest(url:str, term:str) -> Optional[Dict[str,str]]:
    term = (term or "").strip()
    if not term: return None
    j = _req_with_backoff(url, {"q": term}, timeout=12, tries=2)
    # id.loc.gov suggest returns: [query, [labels], [ids], [??], [types]]
    try:
        labels = j[1]; ids = j[2]
        if labels and ids:
            return {"label": labels[0], "uri": ids[0]}
    except Exception:
        return None
    return None
...
def loc_normalize_subjects(subjects: List[str]) -> List[Dict[str,str]]:
    ...
    for s in subjects or []:
        ...
        hit = _loc_suggest(LOC_SUBJ_SUGGEST, s_clean)
        if hit:
...
def loc_normalize_genres(genres: List[str]) -> List[Dict[str,str]]:
    ...
    for g in genres or []:
        ...
        hit = _loc_suggest(LOC_GENRE_SUGGEST, g_clean)
        if hit:
            out.append({"label": hit["label"], "uri": hit["uri"], "scheme": "LCGFT"})
    seen, dedup = set(), []
    for d in out:
        if d["uri"] not in seen:
            seen.add(d["uri"]); dedup.append(d)
...
if __name__ == "__main__":
    print(build_core_metadata("Crime and Punishment", "Fyodor Dostoevsky", 1866))
