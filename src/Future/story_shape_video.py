...
import warnings
import math
import json # Import json to read story data
...
for prefix, uri in NAMESPACES.items(): ET.register_namespace(prefix if prefix != 'svg' else '', uri)

# --- Video.tsx Template (Updated to handle sync text) ---
VIDEO_TSX_TEMPLATE = """\
// {output_filename}
...
const TEXT_REVEAL_SPEED = 1.5;
const TEXT_FADE_DURATION = 15;
const END_DELAY = 15;
...
export const {component_name}: React.FC = () => {{
  ...
  const axesGroupRef = useRef<SVGGElement | null>(null);
  const syncTextElementsRef = useRef<SVGTextElement[]>([]); // Ref for sync text chars
  ...
  useEffect(() => {{
    ...
    // Select Elements
    axesGroupRef.current = svgElement.querySelector<SVGGElement>('#axes-group');
    ...
    const syncTextGroup = svgElement.querySelector<SVGGElement>('#sync-text-group'); // Select sync text group
    ...
    // Select individual sync text elements (assuming they are <text> or <tspan>)
    if (syncTextGroup) {{
        syncTextElementsRef.current = Array.from(
            syncTextGroup.querySelectorAll<SVGTextElement>('text, tspan') // Select text or tspan
        );
        // Optional: Check for length mismatch
        if (mainTextElementsRef.current.length > 0 && syncTextElementsRef.current.length !== mainTextElementsRef.current.length) {{
             console.warn(`Sync warning: Main path elements (${{mainTextElementsRef.current.length}}) != Sync text elements (${{syncTextElementsRef.current.length}})`);
        }}
    }} else {{ syncTextElementsRef.current = []; }}
    ...
    // Animate Main Text Path AND Synchronized Top Text
    mainTextElementsRef.current.forEach((el, i) => {{
      ...
      const currentOpacity = frame >= startRevealFrame ? String(opacity) : '0';

      // Apply to main path element
      if (el instanceof SVGElement && 'style' in el) {{
         (el as SVGGraphicsElement).style.opacity = currentOpacity;
         (el as SVGGraphicsElement).style.stroke = 'none';
      }}

      // --- Apply to corresponding sync text element ---
      const syncTextEl = syncTextElementsRef.current[i]; // Get element by index
      if (syncTextEl && syncTextEl instanceof SVGElement && 'style' in syncTextEl) {{
         (syncTextEl as SVGTextElement | SVGTSpanElement).style.opacity = currentOpacity;
         // You might want to set fill/stroke here too if needed
         // syncTextEl.style.fill = 'yellow'; // Example color
         syncTextEl.style.stroke = 'none';
      }}
      // --- End Sync Text Update ---
    }});
  ...
  useEffect(() => {{
    ...
    const mainTextGroup = svgElement.querySelector<SVGGElement>('#main-text-path');
    // const syncTextGroup = svgElement.querySelector<SVGGElement>('#sync-text-group'); // No need to hide group, individual elements start hidden
    ...
    // Note: Sync text elements are set to opacity="0" in SVG source by Python,
    // so no extra initial hiding is needed here.
  }}, []);
...
def process_svg_structural_v4(input_svg_path, width_in, height_in, dpi, margin_ratio, title_band_height, gap_above_title, story_data_path): # Added story_data_path
    """
    Reads SVG, ADDS axes & sync text groups, identifies Title/Name, Path, restructures.
    """
    try:
        # --- Load original story data to get the text ---
        try:
            with open(story_data_path, 'r', encoding='utf-8') as f:
                story_data = json.load(f)
                # Navigate to the relevant part if nested
                if 'story_plot_data' in story_data: story_data = story_data['story_plot_data']
            # Concatenate the arc text from the components (skip the first 'start' component)
            full_arc_text = "".join([comp.get('arc_text', '') for comp in story_data.get('story_components', [])[1:]])
            if not full_arc_text:
                 print("Warning: Could not extract 'arc_text' from story data JSON. Sync text will be missing.", file=sys.stderr)
            else:
                 print(f"DEBUG: Extracted full arc text ({len(full_arc_text)} chars).")

        except FileNotFoundError:
            print(f"Error: Story data JSON file not found at '{story_data_path}'. Cannot add sync text.", file=sys.stderr)
            full_arc_text = "" # Proceed without sync text
        except Exception as e_json:
             print(f"Error reading or parsing story data JSON '{story_data_path}': {e_json}", file=sys.stderr)
             full_arc_text = ""
        # --- End Load Story Data ---
        ...
        if drawable_width <= 0 or drawable_height <= 0:
             print(f"ERROR: Drawable dimensions non-positive ({drawable_width}x{drawable_height}).", file=sys.stderr); return None
        ...
        axis_horizontal_offset = 40; gi_axis_x_coord = margin_x - axis_horizontal_offset; gi_axis_x = str(gi_axis_x_coord)
        gi_y1 = str(margin_y); gi_y2 = str(margin_y + drawable_height)
        be_axis_y = str(margin_y + drawable_height / 2); be_x1 = gi_axis_x; be_x2 = str(margin_x + drawable_width)
        ET.SubElement(axes_group, f'{{{svg_ns}}}line', attrib={'x1': gi_axis_x, 'y1': gi_y1, 'x2': gi_axis_x, 'y2': gi_y2, 'stroke': axes_color, 'stroke-width': axes_thickness })
        g_label = ET.SubElement(axes_group, f'{{{svg_ns}}}text', attrib={'x': gi_axis_x, 'y': str(margin_y + label_padding), 'fill': axes_color, 'font-size': label_font_size, 'font-family': 'Sans', 'text-anchor': 'middle', 'dominant-baseline': 'hanging'}); g_label.text = "G"
        i_label = ET.SubElement(axes_group, f'{{{svg_ns}}}text', attrib={'x': gi_axis_x, 'y': str(margin_y + drawable_height - label_padding), 'fill': axes_color, 'font-size': label_font_size, 'font-family': 'Sans', 'text-anchor': 'middle', 'dominant-baseline': 'alphabetic'}); i_label.text = "I"
        ET.SubElement(axes_group, f'{{{svg_ns}}}line', attrib={'x1': be_x1, 'y1': be_axis_y, 'x2': be_x2, 'y2': be_axis_y, 'stroke': axes_color, 'stroke-width': axes_thickness})
        b_label = ET.SubElement(axes_group, f'{{{svg_ns}}}text', attrib={'x': str(gi_axis_x_coord + label_padding), 'y': be_axis_y, 'fill': axes_color, 'font-size': label_font_size, 'font-family': 'Sans', 'text-anchor': 'start', 'dominant-baseline': 'middle'}); b_label.text = "B"
        e_label = ET.SubElement(axes_group, f'{{{svg_ns}}}text', attrib={'x': str(margin_x + drawable_width - label_padding), 'y': be_axis_y, 'fill': axes_color, 'font-size': label_font_size, 'font-family': 'Sans', 'text-anchor': 'end', 'dominant-baseline': 'middle'}); e_label.text = "E"

        # --- Create and Add Sync Text Group ---
        sync_text_group = ET.Element(f'{{{svg_ns}}}g', attrib={'id': 'sync-text-group'})
        sync_text_y = str(margin_y - 30) # Position above the top margin (adjust Y offset as needed)
        sync_text_x_start = str(margin_x)
        # Use parameters from the SVG generation if available, otherwise guess
        sync_font_size = "18px" # Make it smaller than axes labels maybe?
        sync_font_family = "Sans" # Or inherit? Use a clear font.
        sync_fill_color = "#E0E0E0" # Light gray/white-ish
        estimated_char_width = 10 # Rough estimate for spacing (adjust)

        if full_arc_text:
            current_x = float(sync_text_x_start)
            for i, char in enumerate(full_arc_text):
                 if char.isspace(): # Handle spaces visually if needed, or just advance x
                      current_x += estimated_char_width * 0.5 # Advance less for space
                      continue
                 char_text_el = ET.SubElement(sync_text_group, f'{{{svg_ns}}}text', attrib={
                      'id': f'sync-char-{i}', # Assign sequential ID
                      'x': str(current_x),
                      'y': sync_text_y,
                      'fill': sync_fill_color,
                      'font-size': sync_font_size,
                      'font-family': sync_font_family,
                      'opacity': '0', # Start hidden
                      'dominant-baseline': 'hanging',
                      'text-anchor': 'start' # Anchor at the start of the char position
                 })
                 char_text_el.text = char
                 # Advance x - this is approximate! Pango gives precise metrics.
                 # Use a fixed width or slightly more for wider chars like 'W', 'M'
                 char_specific_width = estimated_char_width * (1.2 if char in 'MW' else (0.6 if char in 'il.' else 1.0))
                 current_x += char_specific_width * 0.8 # Adjust spacing factor (0.8)

            print(f"DEBUG: Added {len(full_arc_text)} characters to sync-text-group.")
        else:
             print("Warning: No arc text found in JSON, sync-text-group will be empty.")

        # --- Insert Groups ---
        defs_element = root.find(f'{{{svg_ns}}}defs')
        insert_pos_axes = list(root).index(defs_element) + 1 if defs_element is not None else 0
        if background_rect is not None:
            try: bg_index = list(root).index(background_rect); insert_pos_axes = max(insert_pos_axes, bg_index + 1)
            except ValueError: pass
        root.insert(insert_pos_axes, axes_group)
        # Insert sync text *after* axes
        root.insert(insert_pos_axes + 1, sync_text_group)
        print(f"DEBUG: Inserted axes-group at {insert_pos_axes}, sync-text-group at {insert_pos_axes + 1}.")
        parent_map = {c: p for p in root.iter() for c in p} # Rebuild map

        # --- Identify Title and Name Groups ---
        # ... (Same logic as before, find last two <g> excluding axes) ...
        title_group, name_group = None, None
        excluded_tags = {f'{{{svg_ns}}}defs', f'{{{svg_ns}}}metadata', f'{{{svg_ns}}}style'}
        # Exclude both axes and sync text groups when searching for title/name
        direct_g_children = [
             child for child in root
             if child.tag == f'{{{svg_ns}}}g'
             and child.get('id') != 'axes-group'
             and child.get('id') != 'sync-text-group' # Also exclude sync text group
             and child not in excluded_tags
        ]
        ...
        sync_text_descendants = set(sync_text_group.iter()) # Exclude sync text

        print("DEBUG: Starting main path element identification...")
        for elem in root.iter():
            if not is_graphical_element(elem): continue
            if is_inside_defs(elem, parent_map): continue
            if elem == background_rect: continue
            ...
            if elem == sync_text_group or elem in sync_text_descendants: continue # Exclude sync text
        ...
        print(f"DEBUG: Identified {len(main_path_elements)} top-level elements for main-text-path.")
        if not main_path_elements: print("Warning: No elements identified for main text path.", file=sys.stderr)

        # --- Restructure ---
        main_path_parent_g = ET.Element(f'{{{svg_ns}}}g', attrib={'id': 'main-text-path'})
        moved_count = 0
        for elem in main_path_elements[:]:
            parent = parent_map.get(elem)
            if parent is not None and parent != main_path_parent_g:
                try:
                    parent.remove(elem); main_path_parent_g.append(elem)
                    parent_map[elem] = main_path_parent_g; moved_count += 1
                except ValueError: pass
        ...
        insert_index = -1
        insert_target = title_group if title_group is not None else name_group
        # Find position *after* axes and sync text
        last_intro_group_index = -1
        try: last_intro_group_index = list(root).index(sync_text_group) # Should be after axes
        except (ValueError, TypeError):
            try: last_intro_group_index = list(root).index(axes_group) # Fallback to axes
        ...
        if insert_index != -1: # Insert before title/name
             final_insert_pos = min(insert_index, len(list(root)))
             # Make sure it's after axes/sync text
             final_insert_pos = max(final_insert_pos, last_intro_group_index + 1 if last_intro_group_index != -1 else 0)
        else: # Append or insert after axes/sync text
             final_insert_pos = last_intro_group_index + 1 if last_intro_group_index != -1 else len(list(root))

        # Final check on insertion position
        final_insert_pos = min(final_insert_pos, len(list(root)))

        root.insert(final_insert_pos, main_path_parent_g)
        print(f"Info: Inserted 'main-text-path' group at index {final_insert_pos}.")

        # --- Final Sanity Check & Serialize ---
        print("\nDEBUG: Final structure (direct children of root):")
        for i, child in enumerate(root): print(f"  Index {i}: {child.tag}, ID: {child.get('id')}")
        if root.find(f".//{{{svg_ns}}}g[@id='axes-group']") is None: print("ERROR CHECK: axes-group NOT found!")
        if root.find(f".//{{{svg_ns}}}g[@id='sync-text-group']") is None: print("ERROR CHECK: sync-text-group NOT found!")
        if root.find(f".//{{{svg_ns}}}g[@id='main-text-path']") is None: print("ERROR CHECK: main-text-path NOT found!")
        print("-" * 20)

        ET.register_namespace('', NAMESPACES['svg'])
        modified_svg_string = ET.tostring(root, encoding='unicode', method='xml')
        modified_svg_string = re.sub(r'<ns0:', '<', modified_svg_string); modified_svg_string = re.sub(r'</ns0:', '</', modified_svg_string)
        modified_svg_string = re.sub(r'\s*xmlns="[^"]+"', '', modified_svg_string, flags=re.IGNORECASE)
        if 'xmlns="http://www.w3.org/2000/svg"' not in modified_svg_string[:100]: modified_svg_string = modified_svg_string.replace('<svg', '<svg xmlns="http://www.w3.org/2000/svg"', 1)
...
def main():
    parser = argparse.ArgumentParser(description='Add Axes/Sync Text & restructure SVG for Remotion.')
    ...
    parser.add_argument('story_data_json', help='Path to the original story data JSON file (used to get arc text).') # Add this argument
    parser.add_argument('--width', default=8, type=float, required=True, help='Original design width in inches.')
    parser.add_argument('--height', default=10, type=float, required=True, help='Original design height in inches.')
    ...
    parser.add_argument('--title_band_height', type=float, default=150, help='Calculated title band height in pixels (incl padding) (default: 0).')
    parser.add_argument('--gap_above_title', type=float, default=70, help='Gap above title in pixels (default: 20).')

    args = parser.parse_args()

    print(f"Processing SVG (adding axes/sync text): {args.input_svg}")
    modified_svg = process_svg_structural_v4( # Use v4
        args.input_svg, args.width, args.height, args.dpi,
        args.margin_ratio, args.title_band_height, args.gap_above_title,
        args.story_data_json # Pass the JSON path
    )

    if modified_svg:
        # ... (Same debug save, TSX generation, file writing) ...
